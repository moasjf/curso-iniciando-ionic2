...
export class Cards {
  ...
  constructor(public http: Http) {
     ...
     this.db.openDatabase({
         ...
         this.db.executeSql("CREATE TABLE IF NOT EXISTS cards (id INTEGER PRIMARY KEY AUTOINCREMENT, id_desc INTEGER, tipo TEXT, desconto TEXT, categoria TEXT, nome_resumo TEXT, nome_completo TEXT, data_hoje TEXT, validade TEXT, contato TEXT, local_cidade TEXT, local_detalhes TEXT, observacoes TEXT, coordenadas TEXT, img_card TEXT, img_detalhes TEXT)", {}).then((data) => {
  ...
  public adicionar(di) {
    console.log("CARDS: tentei INSERIR: " + JSON.stringify(di) );
     this.db.executeSql("INSERT INTO cards (id_desc, tipo, desconto, categoria, nome_resumo, nome_completo, data_hoje, validade, contato, local_cidade, local_detalhes, observacoes, coordenadas, img_card, img_detalhes) VALUES ( ?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)", [di.id_desc, di.tipo, di.desconto, di.categoria, di.nome_resumo, di.nome_completo, di.data_hoje, di.validade, di.contato, di.local_cidade, di.local_detalhes, di.observacoes, di.coordenadas, di.img_card, di.img_detalhes]).then((data) => {
